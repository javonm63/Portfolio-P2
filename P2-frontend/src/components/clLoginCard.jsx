...
function ClLoginCard({showSignup, closeSignup}) {
    ...
    async function login() {
        const res = await fetch('http://localhost:6001/api/cl/login', {
            method: 'POST', 
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({email: email, pass: pass, role: 'client'}),
            credentials: 'include'
        })
        if (!res.ok) {
            ...
            if (data.message === 'No account') {
                setShowAlert(true)
                setAlertText('No account found, create an client account')
            }
        }
