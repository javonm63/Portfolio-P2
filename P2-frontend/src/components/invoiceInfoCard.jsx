...
function InvoiceInfoCard({name, setName, date, setDate, due, setDue, id, setId}) {
    ...
    return (
        <fieldset className='invoice-page-container'>
            <div className='invoice-info-container'>
                ...
                <section className='invoiceInfoCard-input-cont'>
                    ...
                    <div style={{display: 'flex', alignItems: 'center', width: '100%' }}>
                        <input className='invoiceInfoCard-inputs' type="text" placeholder="Name" required value={name} onChange={(ev) => {setName(ev.target.value)}}></input>
                        <input className='invoiceInfoCard-inputs' type="number" placeholder="Invoice ID" value={id} onChange={(ev) => {setId(ev.target.value)}}></input>
                    </div>
                    <div className='bill-to-info-text-conts'>
                    ...
                    <div style={{display: 'flex', alignItems: 'center', width: '100%' }}>
                        <input className='invoiceInfoCard-inputs' type="date" placeholder="Today's Date" required value={date} onChange={(ev) => {setDate(ev.target.value)}}></input>
                        <input className='invoiceInfoCard-inputs' type="date" placeholder="Due Date" required value={due} onChange={(ev) => {setDue(ev.target.value)}}></input>
                    </div>
                </section>
            </div>
